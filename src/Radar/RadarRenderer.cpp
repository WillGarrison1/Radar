...
void RadarRenderer::AddRadialGeometry(float radialWidth, Radial &radial, std::vector<SDL_Vertex> &vertices, std::vector<int> &indices)
{
    for (auto &gate : radial.gates)
    {
        constexpr float deg2rad = std::numbers::pi / 180.0f;
        auto i = gate.gateNum;
        float az1 = (90.0f - radial.trueAzimuth - radialWidth / 2) * deg2rad; // half beamwidth
        float az2 = (90.0f - radial.trueAzimuth + radialWidth / 2) * deg2rad;
...
void RadarRenderer::CreateScanGeometry(RadarScan &scan, std::vector<SDL_Vertex> &vertices, std::vector<int> &indices)
{
    ...
    {
        AddRadialGeometry(scan.radialWidth, radial, vertices, indices);
    }
}

void RadarRenderer::OnKeyPress(SDL_Event &e)
{
    if (e.key.key == SDLK_W)
    {
        metersPerPixel -= 10;
...
void RadarRenderer::UpdateEvents()
{
    SDL_Event e;
...
void RadarRenderer::Update(float deltaTime)
{
    ...
    if (cache.contains(timePoint) && !cache[timePoint].radials.empty())
    {
        ...
        CreateScanGeometry(scan, vertices, indices);
